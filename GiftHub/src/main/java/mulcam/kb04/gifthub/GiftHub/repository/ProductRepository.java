...
public interface ProductRepository extends JpaRepository<Product, Integer> {
	...
	@Query(value = "SELECT p.PRODUCTNO, p.PRODUCTPRICE, p.PRODUCTNAME, p.PRODUCTMEMO, p.PRODUCTEXP, p.PRODUCTIMAGE, p.STOREID, s.CATEGORYNO, s.STORENAME FROM PRODUCT p JOIN STORE s ON p.STOREID = s.STOREID WHERE p.PRODUCTEXP >= SYSDATE", nativeQuery = true)
